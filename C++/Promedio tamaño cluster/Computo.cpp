...
int main(int argc,char **argv){
	...
	ofstream Outfile;
	for (int l=40;l<=40;l++){	//este l varía el número de filas y columnas
		...
		for (int t=1;t<=np; ++t){		//Este t hace variar la probabilidad de ocupación
			...
			int contador=0;			//Contador del número del tamaño de cluster al hacer num iteraciones. Primero lo seteo en 0
			for (int h=1; h<=num; ++h){		//Este h hace que el sistema se repita num-veces
				...
				while (gate==1){		//Este while puede llegar a no salirse del loop, pero es poco probable
					...
					y_1=rand()%nyC+1;		//Seteo y_1 como un número aleatorio entre 1 y nyC
						if (gameState[x_1][y_1]==1){		//Si la celda está ocupada
							gate=0;							//Seteo la gate para que salga del loop del while
						}
				}

				//Ahora analizo cuántas celdas hay en el mismo cluster que la celda al azar, osea que
				//sumo en una unidad el contador por cada celda que tenga la misma etiqueta que la celda al azar


				for (int x=1; x<=nxC; x++){
					for (int y=1; y<=nyC; y++){
						if (label[x][y]==label[x_1][y_1]){
							contador++;
						}
					}
				}
			}
			Outfile << p << " " << (double)contador/num << endl;		//Escribo la probabilidad de percolación 
																		//seguido del promedio del tamaño del cluster esperado
		}
